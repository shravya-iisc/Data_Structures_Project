...
# These tests validate SCC properties using random directed graphs generated by Hypothesis.
# They verify correctness of SCC decomposition, mutual reachability, partitioning, and condensation.
...
def test_scc_dag_singleton_components(G):
    ...
    if nx.is_directed_acyclic_graph(G):
        ...
        print("FAIL: Graph is not acyclic; singleton-SCC property not applicable")


@given(directed_graphs())
def test_condensation_node_count_equals_scc_count(G):
    """
    Property: The condensation graph has one node per SCC.

    Mathematical Basis:
    The condensation graph collapses each strongly connected component into a single supernode.
    Therefore the number of nodes in the condensation must equal the number of SCCs.

    Test Strategy:
    Compute SCCs and the condensation graph, then compare their node counts.

    Why this is important:
    This ensures the condensation mapping is complete and does not lose or duplicate SCCs.
    """
    sccs = list(nx.strongly_connected_components(G))
    condensation = nx.condensation(G)

    assert condensation.number_of_nodes() == len(sccs), \
        "FAIL: Condensation node count differs from SCC count"

    print("PASS: Condensation node count matches SCC count")


@given(directed_graphs())
def test_each_scc_is_strongly_connected(G):
    """
    Property: Every computed SCC induces a strongly connected subgraph.

    Mathematical Basis:
    By definition, an SCC is a maximal set of nodes where each pair is mutually reachable.

    Test Strategy:
    For each SCC, construct the induced subgraph and verify it is strongly connected.

    Why this is important:
    This confirms the SCC algorithm returns components that satisfy the core strong connectivity requirement.
    """
    sccs = list(nx.strongly_connected_components(G))
    for comp in sccs:
        subgraph = G.subgraph(comp)
        assert nx.is_strongly_connected(subgraph), \
            f"FAIL: SCC {comp} is not strongly connected"

    print("PASS: Every SCC is strongly connected")
...
def test_adding_edge_does_not_decrease_flow(data):
    ...
    if not G2.has_edge(s, t):
        ...
        print("PASS: Direct source→sink edge already exists; max flow did not decrease")
